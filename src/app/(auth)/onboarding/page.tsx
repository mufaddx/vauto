...
export default function OnboardingPage() {
  ...
  /** Each step is persisted, so a reload resumes where the user left off. */
  function advance(next: number) {
    const form = new FormData();
    form.set("accountType", accountType);
    form.set("step", String(next));
    startTransition(async () => {
      await saveOnboardingStep(form);
      setStep(next);
    });
  }
    ...
    <div className="w-full max-w-lg rounded-3xl border border-border bg-card p-8 shadow-[var(--shadow)]">
      ...
      {step === 1 ? (
        <>
            ...
            {types.map((type) => (
              <button
                ...
                className={`rounded-2xl border px-4 py-4 text-sm font-medium ${
                  accountType === type ? "border-accent bg-accent-soft" : "border-border"
                }`}
              >
                {labels[type]}
              </button>
          ...
          <Button className="mt-6 w-full" disabled={pending} onClick={() => advance(2)}>
      ...
      {step === 2 ? (
        <>
          ...
          <p className="mt-2 text-sm text-secondary">
            You can skip this and connect later from Channels. Creators are not
            required to enter business details.
          </p>
          <div className="mt-6 space-y-3">
            <Link
              href="/api/channels/instagram/connect"
              className="block rounded-2xl border border-border px-4 py-4 hover:border-accent"
            >
              Instagram
            </Link>
            <Link
      ...
      {step === 3 ? (
        <>
          ...
          <p className="mt-2 text-sm text-secondary">
            Optional now. You can start from a campaign after you connect a channel.
          </p>
          <div className="mt-6 flex gap-3">
            <Button
              variant="secondary"
        ...
        <>
          ...
          <p className="mt-2 text-sm text-secondary">
            {accountType === "BUSINESS"
              ? "Activate Business Information before automations use global facts."
